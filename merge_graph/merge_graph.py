...
def get_subgraph(fact_in_list,fact_out_list,json_dic_path,contained_file=None):
    if contained_file:
    ...
    for json_file in json_files:
        with open(json_dic_path+json_file,"r") as file:
            ...
            if all(fact_id in fact_id_list for fact_id in fact_in_list) and all(fact_id in fact_id_list for fact_id in fact_out_list):
                ...
                from_subgraph,_ = get_subgraph_from_list(fact_out,op_nodes)
                to_subgraph,_ = get_subgraph_to_list(fact_in,op_nodes)
                ...
                if all(fact_id in subgraph for fact_id in fact_in_list) and all(fact_id in subgraph for fact_id in fact_out_list):
                    ...
                    for factid in subgraph:
                        ...
                        if factid not in fact_id_cnt:
                            ...
                            fact_id_cnt[factid] += 1
    return contained_fact,fact_id_cnt

## 判断max-fact-pair的逻辑合理性
def judge_logic(max_fact_id_list):
    pass

# well-tested(partly)
# 选择max_fact_pair
def select_max_fact_id(fact_id_cnt,contained_fact):
    max_fact_id_list = []
    max_cnt = 0
    ...
    while True:
        ...
        if not is_continue:
            break
    judge_logic(max_fact_id_list)
    max_fact_id = []
    for fact_id in max_fact_id_list:
        if len(fact_id) > len(max_fact_id):
            max_fact_id = fact_id
    contained_file = []
    not_contained_file = []
...
# 从初始图得到完整图
def merge_graph(graph:Graph,contained_fact,fact_id_cnt,json_dic_path):
    if fact_id_cnt == {}:
        return
    ...
    while len(not_contained_file) > 0:
        contained_fact,fact_id_cnt = get_subgraph(graph.start_node.get_fact(),graph.end_node.get_fact(),json_dic_path,not_contained_file)
        ...
        # print('xxxxx')
        ## 左侧
        left_contained_fact,left_fact_id_cnt = get_subgraph(graph.start_node.get_fact(),max_fact_id,json_dic_path,contained_file)
        # print("left: ",left_contained_fact,left_fact_id_cnt)
        left_subgraph = Graph(graph.start_node,new_node)
        merge_graph(left_subgraph,left_contained_fact,left_fact_id_cnt,json_dic_path)
        graph.merge(left_subgraph)
        # print('yyyyyyyyyy')
        # graph.display()
        # print('yyyyyyyyyy')
        ## 右侧
        right_contained_fact,right_fact_id_cnt = get_subgraph(max_fact_id,graph.end_node.get_fact(),json_dic_path,contained_file)
        right_subgraph = Graph(new_node,graph.end_node)
        merge_graph(right_subgraph,right_contained_fact,right_fact_id_cnt,json_dic_path)
        graph.merge(right_subgraph)
    return

# 得到最终的结果图
def get_final_graph(fact_in_list,fact_out_list,json_dic_path):
    contained_fact,fact_id_cnt = get_subgraph(fact_in_list,fact_out_list,json_dic_path)
    if contained_fact == {}:
        return None
    graph = initial_graph(fact_in_list,fact_out_list,contained_fact)
    # print('initial graph')
    # graph.display()
    merge_graph(graph,contained_fact,fact_id_cnt,json_dic_path)
    return graph
